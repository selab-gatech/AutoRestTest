...
from autoresttest.models import OperationProperties, SchemaProperties, SimilarityValue
...
class OperationDependencyComparator:
    ...
    def get_parameter_list(
        ...
        if operation.parameters is None:
            return []
        return [
            {self.model.handle_word_cases(parameter): parameter}
    ...
    def cosine_similarity(
        self,
        operation1_vals: List[Dict[str, str]],
        operation2_vals: List[Dict[str, str]],
        in_value: str = None,
    ) -> Dict[str, List[SimilarityValue]]:
        """
            ...
            for processed_parameter, parameter in parameter_pairing.items():
                ...
                for dependency_pairing in operation2_vals:
                    for processed_dependency, dependency in dependency_pairing.items():
                        ...
                        if (
                            param_embedding is not None
                            and dependency_embedding is not None
                        ):
                            similarity = 1 - cosine(
                                param_embedding, dependency_embedding
                            )
                            param_param_similarity[parameter].append(
                                SimilarityValue(
    ...
    def compare_cosine(
    ...
    ) -> Tuple[Dict[str, List[SimilarityValue]], List[Tuple[str, SimilarityValue]]]:
        ...
        if operation1.parameters:
            if operation2.parameters:
                parameter_matchings = self.cosine_similarity(
                    operation1_parameters,
                    operation2_parameters,
                    in_value="query to query",
                )
            if operation2.request_body:
                added_parameter_matchings = self.cosine_similarity(
                    operation1_parameters, operation2_body, in_value="query to body"
                )
        ...
        if operation1.request_body:
            if operation2.parameters:
                added_parameter_matchings = self.cosine_similarity(
                    operation1_body, operation2_parameters, in_value="body to query"
                )
                for parameter, similarities in added_parameter_matchings.items():
                    parameter_matchings.setdefault(parameter, []).extend(similarities)
            if operation2.request_body:
                added_parameter_matchings = self.cosine_similarity(
                    operation1_body, operation2_body, in_value="body to body"
                )
                for parameter, similarities in added_parameter_matchings.items():
                    parameter_matchings.setdefault(parameter, []).extend(similarities)
            if operation2.responses:
                added_parameter_matchings = self.cosine_similarity(
                    operation1_body, operation2_responses, in_value="body to response"
                )
                for parameter, similarities in added_parameter_matchings.items():
                    parameter_matchings.setdefault(parameter, []).extend(similarities)
