...
#Install RestTestGen
cd $DEFAULT_DIR
source ./java11_arm.env && cd tools/resttestgen && ./gradlew install
...
#Install Evo Bench
cd $DEFAULT_DIR
source ./java8_arm.env && cd services/emb && mvn clean install -DskipTests && mvn dependency:build-classpath -Dmdep.outputFile=cp.txt

#Install Genome-Nexus
cd $DEFAULT_DIR
source ./java8_arm.env && cd services/genome-nexus && mvn clean install -DskipTests

#Install YouTube Mock service
cd $DEFAULT_DIR
source ./java11_arm.env && cd services/youtube && mvn clean install -DskipTests && mvn dependency:build-classpath -Dmdep.outputFile=cp.txt
